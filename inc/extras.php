...
if ( ! function_exists( '_wp_render_title_tag' ) ) :
	 ...
	 */
	function artist_wp_title( $title, $sep ) {
		if ( is_feed() ) {
			return $title;
		}
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf( __( 'Page %s', 'artist' ), max( $paged, $page ) );
		}

		return $title;
	}
	add_filter( 'wp_title', 'artist_wp_title', 10, 2 );
endif;
...
if ( ! function_exists( '_wp_render_title_tag' ) ) :
	 ...
	 */
	function artist_render_title() {
		echo '<title>' . wp_title( '|', false, 'right' ) . "</title>\n";
	}
	add_action( 'wp_head', 'artist_render_title' );
endif;
